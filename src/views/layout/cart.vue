  ...
  <div class="cart">
    ...
    <div class="footer-fixed">
      ...
      <div class="all-total">
        ...
        <div
         v-if="!isEdit" class="goPay" :class="{disabled:selCount === 0}"
        @click="goPay"
        >结算({{selCount}})</div>
...
export default {
  ...
  methods: {
    ...
    changeCount (goodsNum, goodsId, goodsSkuId) {
      ...
      // 调用 vuex 的action， 进行数据的修改
      this.$store.dispatch('cart/changeCountAction', {
        goodsNum, goodsId, goodsSkuId
      })
    },
    async handleDel () {
      if (this.seleCount === 0) return
    ...
    goPay () {
      ...
      if (this.selCount > 0) {
        ...
        this.$router.push(({
          ...
          query: {
            mode: 'cart',
            cartIds: this.selCartList.map(item => item.id).join(',')// 格式 'cartId,cartId,cartId'
          }
        }))
      }
    }
